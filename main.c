...
void transformaModeloDual(int **mtzCoeficientes,int qtdL ,int qtdC , int *funcObjeva,int qtdFO, int *igualdadeModelo,int qtdIGM){
	...
	system("cls");	
	printf("Modelo na forma DUAL: \n\n");
	apresentaValoresMatriz(mtzDual,auxL,auxC);
	getch();
}

//Objetivo: Apresentar matrizes que comp?em o modelo
//Parametros: Matriz com valores do modelo, vetor com variaveis de
//Retorno: Vetor com devida quantidade de posi??es
void apresentaMtzModelo(int **mtzCoeficientes, int *funcObjeva, int qtdL, int qtdC){
	system("cls");
...
//Objetivo: Apresentar valores da fun??o objetiva
...
//Objetivo: Aloca??o din?mica para vetor
//Parametros: Quantidade de posi??es que o vetor deve possuir
//Retorno: Vetor com devida quantidade de posi??es
int *alocaVetor(int qtd){
...
//Objetivo: Aloca??o din?mica para matriz 
...
char ** alocaMatrizChar(int qtdL, int qtdC){
	...
	for (i = 0; i < qtdL; i++){ //Percorre as linhas do Vetor de Ponteiros
		mtz[i] = (char*) malloc(qtdC * sizeof(char)); //Aloca um Vetor de Inteiros para cada posi??o do Vetor de Ponteiros.
...
int ** alocaMatriz (int qtdL, int qtdC){
	...
	for (i = 0; i < qtdL; i++){ //Percorre as linhas do Vetor de Ponteiros
		mtz[i] = (int*) malloc(qtdC * sizeof(int)); //Aloca um Vetor de Inteiros para cada posi??o do Vetor de Ponteiros.
		for (j = 0; j < qtdC; j++){ //Percorre o Vetor de Inteiros atual.
			mtz[i][j] = 0; //Inicializa com 0.
		}
	}
...
//Parametros: Matriz de vakires para modelo, vetor de fun??o objetiva, vetor de igualdades do modelo, quantidade de linhas e colunas da matriz
void populaDados(int **mtz,int *funcObjeva,int *igualdadeModelo,int qtdL, int qtdC){
...
//Objetivo: Verificar qual Operador matem?tico a string representa 
//Par?metro: String 
//Retorno: Inteiro correspondente: MENOR_IGUAL 3, MAIOR_IGUAL 2, IGUAL_IGUAL 1, retorna 0 se n?o for um operador
...
//Parametros: Vetor, quantidade de posi??es
void apresentaValoresVetor(int *vet, int qtdP){
	int x; 
...
// Par?metros: endereco de memoria  da string  mensagem, mensagem de erro e das opcoes validas e endere?o da opcao
